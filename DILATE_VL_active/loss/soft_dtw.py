...
def pairwise_distances(x, y=None):
    ...
    '''
    if y is not None:
        p1, p2 = torch.broadcast_tensors(x.unsqueeze(1), y.unsqueeze(0))
        dist=(-1*torch.mean(p2*torch.log(p1*(1-9*epsilon)+epsilon),-1)+torch.mean(p2*torch.log(p2*(1-9*epsilon)+epsilon),-1))*2
    else:
        x_norm = (x**2).sum(1).view(-1, 1)
        y_t = torch.transpose(x, 0, 1)
        y_norm = x_norm.view(1, -1)
